versionNumber = { version: 34 } // to reset file cache forefully on prod. Updated with shell script.
...
// ======= BASIC HTML FOR ADDITIONS / REPLACEMENTS =======
  ...
  if ( currentPathX() !== '' && currentPathX() !== 'about-us' && !== 'locations' ) { $('div.sqs-block-content p.text-align-center').eq(1).before(formHtml) }
  if ( currentPathX() !== '' && currentPathX() !== 'about-us' && === 'locations') { $('div#canvas div.sqs-layout.sqs-grid-12 div.row.sqs-row div.col.sqs-col-12').prepend(formHtml).after('<p>placeholder for email in a few lines</p>') }
