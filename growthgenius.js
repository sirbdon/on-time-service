versionNumber = { version: 136 } 

// HTML snippet gets
var
  blankDiv = $('<div class="blankDiv">'),
  helpHtml = 'https://sirbdon.github.io/on-time-service/helpers.html',
  phoneMenuImages = blankDiv.load(helpHtml + ' #helper-menu-imgs');
  // formHtml = blankDiv.load(helpHtml + ' #formHtml');
...
// Add note to locations page
  var 
    locationRoot   = (window.location.href === 'http://ontimeserviceco.com/locations/'),
    locationPage   = (window.location.pathname === '/locations/'),
    locationInsert = $('#canvas').find('.page-divider.top-divider');
  ...
  var subtitle = '<p id="location-subtitle" style="font-size:18px;margin-top: 3%;margin-bottom: -3%;margin-left:auto;margin-right:auto;text-align:center;">\
                    Or click on your province below to find your nearest On Time Service counter located within <strong>The Hudson\'s Bay</strong>\
                  </p>';
  ...
  // remove 'subtitle' if not on location root page + remove workshop location if not in BC
  $('a.project div.content-fill').click(function() {
    if (locationRoot) { $('#location-subtitle').remove() }
    if ( $(this).parents('a.project').attr('href') !== '/british-columbia/' ) { $('#workshopLoc').remove() }
  })
...
//   $('.buyButton').remove() // fix to remove button on fast mouse move
...
<div class="row">\
  <div class="col-md-12">\
    <h1><strong>On Time \nWatch & Jewellery Service</strong></h1>\
  </div>\
</div>\
<div class="row">\
  <div class="col-md-12">\
    <p style="font-size:25px;margin-bottom0px;">Family Owned & Operated</p>\
  </div>\
</div>\
<div class="row">\
  <div class="col-md-12">\
...
<br>\
<div class="row">\
  <h2>Convenience, service and expertise</h2>\
</div>\
<br>\
<div class="row">\
  <div class="col-md-4">\
...
<div class="row" style="text-align:left">\
  ...
  <div class="col-md-8 pull-right">\
    <p class="homeDetails"><strong>Complete watch servicing</strong>, from full rebuilds and technical revisions all the way down to battery replacement and ultrasonic cleaning.</p>\
  </div>\
  <div class="col-md-8 pull-right">\
    <p class="homeDetails"><strong>Anything you can imagine we can do with jewellery.</strong> From completely custom build rings, converting heirlooms into a modern brooch all the way to chain repairs, ring sizing, and chain soldering.<p>\
  </div>\
  <div class="col-md-8 pull-right">\
    <p class="homeDetails"><strong>A large variety of accessories and other services</strong> are also available. We\'re confident we can meet any of your watch and jewellery desires. Please reach out using the form at the top or at the email below.</p>\
  </div>\
</div>\
</div>'
...
function screenAdjusts() {
  ...
  if ( isPhoneX() ) { 
    ...
    // $('#helper-menu-imgs').unwrap()
    $('#mobileMenuLink > a').remove()
    $('#mobileMenuLink').prepend($('<a style="display:none">'))
    $('#footerBlock .sqs-block-content > p').addClass('phonex-footer')
  }
  if ( !isPhoneX() ) { 
    $('div.site-phone').replaceWith(divSitePhone)
    $('#footerBlock .sqs-block-content > p').removeClass('phonex-footer')
  }
...
// underline nav items
// $('#helper-menu-imgs img.pull-left').click(function() {
